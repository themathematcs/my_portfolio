...
//   // ⚠️ CHANGE 2: Updated handleSend for Streaming
...
//     // 2. ADD THIS: Add an EMPTY Model Message (placeholder for the stream)
...
const AIChat: React.FC<AIChatProps> = ({ onClose }) => {
  ...
  const handleSend = async () => {
    ...
    // 2. Add an EMPTY Model Message (placeholder for the stream)
    setMessages(prev => [...prev, { role: 'model', text: '' }]);
  ...
  return (
    <>
      ...
      <motion.button
        ...
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.9 }}
        onClick={() => setIsOpen(!isOpen)} // ⬅️ FIX: TOGGLE THE STATE
        initial={{ scale: 0 }}
        animate={{ scale: 1 }}
      >
        <MessageSquare size={24} />
          ...
          >
            ...
            <div className="p-4 bg-white/5 backdrop-blur-md flex justify-between items-center border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-lg font-semibold text-slate-100">AI Assistant</h4>
              </div>
